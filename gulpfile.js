...
function html () {
    return src('src/pages/index.html') // Выбираем источник: "app/sass/main.sass" или "app/less/main.less"
        .pipe(gulpRigger()) //Прогоним через rigger
        .pipe(dest('./')) // Выгрузим результат в папку "app/css/"
        .pipe(browserSync.stream()) // Сделаем инъекцию в браузер
}
...
exports.build = series(cleandist, styles, html, scripts, images, buildcopy);
